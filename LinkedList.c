...
struct LinkedList *addend(struct LinkedList *head)
{
    int x;
    printf("enter the data :");
    scanf("%d", &x);
    struct LinkedList *ptr = head;
    struct LinkedList *flot = (struct LinkedList *)malloc(sizeof(struct LinkedList));
    flot->data = x;
    while (ptr->next != 0)
    {
        ptr = ptr->next;
...
int main()
{
    ...
    trav(head);
    printf("\nQ4:- add a new element at given index in linkedlist :\n");
    head = add(head);
    printf("Element added Successfully\n");
    printf("New elements of linked list :\n");
    trav(head);
